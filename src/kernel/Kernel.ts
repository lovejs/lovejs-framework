...
export class Kernel {
    ...
    async registerPlugin(plugin, pluginsNames) {
        if (this.booted) {
            throw new PluginError(`Cannot add a plugin on an already booted Kernel`, { plugin });
        }
        ...
        if (!modulePath) {
            throw new PluginError(
                `Unable to find the plugin '${plugin}', neither as module "@lovejs/${plugin}", "lovejs-${plugin}" or as local plugin in "./plugins/${plugin}" project directory`,
                { plugin }
            );
        }
        ...
        if (await fileExists(schemaPath)) {
            ...
            try {
                ...
                throw new PluginError(`Configuration error validating configuration from "${this.getPluginConfigFile(plugin)}"`, {
                    plugin,
                    error
                });
            }
    ...
    async getPluginConfig(plugin) {
        try {
            ...
            throw new PluginError(`Error loading configuration`, { plugin, error });
        }
    }
        ...
        for (let p of this.getPlugins()) {
            ...
            if (plugin.registerServices) {
                if (!_.isFunction(plugin.registerServices)) {
                    throw new PluginError(`Error register services, the "registerServices" property on plugin class must be a function`, {
                        plugin: name
                    });
                }
                try {
                    ...
                    throw new PluginError(`Error registering services`, { plugin: name, error });
                }
            }
        }
    }
